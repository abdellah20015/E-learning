...
<?php
        ...
        $sql = "SELECT * FROM exercices WHERE titre LIKE :q OR description LIKE :q";
    ...
    while ($row = $stmt->fetch()) {
      ...
      if ($type === '' || (isset($row["type"]) && $row["type"] == $type)) {
        // Afficher le cours
        echo "<div class='exercice-tile'>";
        echo "<img src='data:image/jpeg;base64," . base64_encode($row['image']) . "' alt='Image d'exercice'>";
        
        echo "<a href='exercice{$row["id"]}.html' class='unique-link'><i class='fa fa-plus'></i> {$row["titre"]}</a>";
        echo "<style>
        .unique-link {
          display: inline-block;
          padding: 10px 20px;
